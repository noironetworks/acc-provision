...
def config_adjust(args, config, prov_apic, no_random):
    ...
    apic_request_retry_delay_base = config["kube_config"].get(
        "apic_request_retry_delay_base", 20)
...
def config_validate(flavor_opts, config):
    ...
    def required(x):
        return True if x else Raise(Exception("Missing option"))
    ...
    def isname(x, l):
        return (1 < len(x) < l) and \
            x[0].isalpha() and x.replace('_', '').isalnum() \
            if x else Raise(Exception("Invalid name"))

    def get(t):
        return functools.reduce(lambda x, y: x and x.get(y), t, config)
