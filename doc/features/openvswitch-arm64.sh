...
docker buildx build "$BUILDARG" --no-cache --platform linux/arm64 -t "$DOCKER_HUB_ID/openvswitch-base:$DOCKER_TAG" -f ./build/openvswitch/Dockerfile-openvswitch-base build/openvswitch &> /tmp/openvswitch-base.log &
...
docker buildx build "$BUILDARG" --no-cache --platform linux/arm64 -t "$DOCKER_HUB_ID/openvswitch:$DOCKER_TAG" -f ./build/openvswitch/Dockerfile-openvswitch build/openvswitch/dist
