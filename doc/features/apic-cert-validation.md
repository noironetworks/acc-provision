...
1.  **Enhanced Security:**
    Enables validation for all API calls to the APIC. This prevents man-in-the-middle (MITM) attacks by ensuring the controller and provisioning script are communicating with a legitimate APIC.
...
3.  **Full Lifecycle Validation:**
    Validation is enforced during both the provisioning & the unprovision phases (by the `acc_provision` script) and during runtime operations (by the `aci-containers-controller`).
...
1.  **Provisioning Script (`acc_provision.py`):**
    *   When the script is run (e.g., with `-a` or `-d` flags), it uses the file **path** provided in `apic_tls_cert` to perform its own verification for any API calls it makes to the APIC.
    *   The script also reads the **content** of the certificate file and prepares it to be embedded in the generated Kubernetes manifests.
...
2.  **Run the Provisioning Script:**
    ...
    Execute the script as usual to generate the new deployment YAML.

    ```sh
    python3 acc_provision.py -c acc-provision-input.yaml -f <your-flavor> -o aci-deployment.yaml
    ```
...
3.  **Apply and Verify:**
    ...
    *   **Check the controller pod for the mounted file:**
        ```sh
        $ kubectl -n aci-containers-system exec -it <aci-containers-controller-pod-name> -- ls /usr/local/etc/apic-ca-cert/
        root_ca.pem
        ```